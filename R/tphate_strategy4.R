#' Transition PHATE : Strategy 4
#' 
#' scale down the voxel activation affinity by transition probability
#' 
#' 
#' @export
tphate_strategy4 <- function(data, ndim=2, nbdk=5, alpha=2.0, symmetrize=TRUE){
  # Inputs
  N = base::nrow(data)
  ...
  # construct : scale the elements
  affinity_machine = aff_tmp
  for (i in 1:N){
    for (j in 1:N){
      state_i = round(aff_lab[i])
      state_j = round(aff_lab[j])
      affinity_machine[i,j] = state_transition[state_i,state_j]*affinity_machine[i,j]
    }
  }
