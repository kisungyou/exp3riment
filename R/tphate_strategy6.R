...
tphate_strategy6 <- function(data, ndim=2, nbdk=5, alpha=2.0, scaler=0.5){
  ...
  for (i in 1:(nulabel-1)){
    id_i = which(aff_lab==ulabel[i])
    for (j in (i+1):nulabel){
      id_j   = which(aff_lab==ulabel[j])
      maxval = max(aff_temporal[id_i,id_j])
      aff_temporal[id_i,id_j] = maxval
      aff_temporal[id_j,id_i] = maxval
    }
  }
